...
INTERNAL_BROKER_LISTENER_NAME="INTERNAL_${BROKER_ID}"
...
do
  ...
  EXTERNAL_MAP_KEY="EXTERNAL_${i}";
  CONTROLLER_MAP_KEY="CONTROLLER_${i}";
  LISTENER_SECURITY_PROTOCAL_MAP="$LISTENER_SECURITY_PROTOCAL_MAP,$INTERNAL_MAP_KEY:PLAINTEXT,$EXTERNAL_MAP_KEY:PLAINTEXT,$CONTROLLER_MAP_KEY:PLAINTEXT"
  if (( $i == 0 )); then
#    CONTROLLER_LISTENER_NAMES="CONTROLLER_${i}"
    CONTROLLER_QUORUM_VOTERS="$i@$DNS_NAME:$CONTROLLER_PORT"
#    ADVERTISED_LISTENERS="$EXTERNAL_MAP_KEY://$DNS_NAME:$EXTERNAL_BROKER_PORT,$INTERNAL_MAP_KEY://$DNS_NAME:$INTERNAL_BROKER_PORT"
#    LISTENERS="$INTERNAL_MAP_KEY://$DNS_NAME:$INTERNAL_BROKER_PORT,$EXTERNAL_MAP_KEY://$DNS_NAME:$EXTERNAL_BROKER_PORT,$CONTROLLER_MAP_KEY://$DNS_NAME:$CONTROLLER_PORT"
  else
...
#    ADVERTISED_LISTENERS="$ADVERTISED_LISTENERS,$EXTERNAL_MAP_KEY://$DNS_NAME:$EXTERNAL_BROKER_PORT,$INTERNAL_MAP_KEY://$DNS_NAME:$INTERNAL_BROKER_PORT"
#    LISTENERS="$LISTENERS,$INTERNAL_MAP_KEY://$DNS_NAME:$INTERNAL_BROKER_PORT,$EXTERNAL_MAP_KEY://$DNS_NAME:$EXTERNAL_BROKER_PORT,$CONTROLLER_MAP_KEY://$DNS_NAME:$CONTROLLER_PORT"
  fi
